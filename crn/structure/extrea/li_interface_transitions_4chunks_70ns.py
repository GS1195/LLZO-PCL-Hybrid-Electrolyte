...
Outputs per chunk counts in the format:
  L(P->S), L(S->P), R(P->S), R(S->P)

Also writes a LaTeX table matching your manuscript style.
"""
...
def split_frames_3x70_plus_remainder(frames, dt_ps, stride, chunk_ns=70.0, n_fixed=3):
    ...
    Chunks can be empty if trajectory is short; later we skip chunks with <2 frames.
    """
    if len(frames) == 0:
        return []
    ...
    # elapsed time for N frames is (N-1)*dt_eff_ps, so choose:
...
def analyze_chunk(u, solid, groups, frames_chunk, chunk_id, dt_ps, stride,
    ...
    """
    Returns:
      by_side_sum: direction->side->count summed over selected species
      events: per hop rows
      time_ns: observed chunk time (ns)
    """
    by_side = {  # sums over species
        "P->S": {"-": 0, "+": 0},
        "S->P": {"-": 0, "+": 0},
    }
    events = []
    ...
    for f in frames_chunk:
        ...
        # absolute time from global start frame
...
def write_latex_table(path_tex, T_K, chunk_rows, caption_extra=""):
    ...
    chunk_rows: list of dicts with keys:
      chunk_label, time_ns, L_P2S, L_S2P, R_P2S, R_S2P
    """
    # Format the temperature header similar to user's example.
    # Here chunk times differ (C4 remainder), so we put nominal in parentheses as "C1..C4 times".
    ...
    lines.append(r"\textbf{T (K) [ns/chunk]} & \textbf{Chunk} & "
                 ...
                 r"\textbf{R (P$\to$S)} & \textbf{R (S$\to$P)} \\ \hline")

    lines.append(rf"\multirow{{{len(chunk_rows)}}}{{*}}{{{T_header}}} "
                 rf"& {chunk_rows[0]['chunk_label']} & {chunk_rows[0]['L_P2S']} & {chunk_rows[0]['L_S2P']} & {chunk_rows[0]['R_P2S']} & {chunk_rows[0]['R_S2P']} \\ \cline{{2-6}}")

    for r in chunk_rows[1:]:
        lines.append(rf" & {r['chunk_label']} & {r['L_P2S']} & {r['L_S2P']} & {r['R_P2S']} & {r['R_S2P']} \\ \cline{{2-6}}")

    lines[-1] = lines[-1].replace(r"\cline{2-6}", r"\hline")  # close block nicely
...
def main():
    ...
    # Atom types
    ap.add_argument("--li_types", default="16,17", help="Comma-separated Li atom types to include (sum together)")
    ...
    u = mda.Universe(args.top, args.traj, topology_format="DATA", format="LAMMPSDUMP", dt=args.dt_ps)
    solid = u.select_atoms(args.solid_sel)
    if solid.n_atoms == 0:
        raise RuntimeError("Solid selection returned 0 atoms. Check --solid_sel.")

    # Build groups (sum over these later)
    groups = {}
    for t in li_types:
    ...
    # Exactly 4 chunks: 3×70 ns + remainder
    ...
    for cid, fr in enumerate(chunks, start=1):
        if len(fr) < 2:
            # still record as zero chunk (for fixed 4-chunk bookkeeping)
            chunk_rows.append({
                "chunk_id": cid,
                "chunk_label": f"C{cid}",
                "time_ns": 0.0,
    ...
    # Write CSV (table-ready)
    out_csv = "chunk_transition_counts.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["T_K","chunk","time_ns","L_P2S","L_S2P","R_P2S","R_S2P"])
        for r in chunk_rows:
            w.writerow([args.T, r["chunk_label"], f"{r['time_ns']:.6f}",
                        r["L_P2S"], r["L_S2P"], r["R_P2S"], r["R_S2P"]])

    # Write LaTeX table matching your layout
    out_tex = "chunk_transition_counts.tex"
    write_latex_table(out_tex, args.T, chunk_rows)

    # Optional: per-hop events (kept for audit)
    out_events = "li_transitions_events.csv"
    ...
    # Console summary: print exactly the table rows
    print("\n=== Chunk transition counts (table format) ===")
    print("T(K)  Chunk  time(ns)   L(P->S)  L(S->P)  R(P->S)  R(S->P)")
    for r in chunk_rows:
        print(f"{args.T:<5d}  {r['chunk_label']:<5s}  {r['time_ns']:>8.2f}  "
              f"{r['L_P2S']:>7d}  {r['L_S2P']:>7d}  {r['R_P2S']:>7d}  {r['R_S2P']:>7d}")
