...
• Y-axis:      (F(z) - F(0))/k_B  for each curve (same reference F(0) per curve).
...
def main():
    ...
    for c in curves:
        ...
        # print summary
        print(
            f"{label}: F(0) = {F0: .3e} J (= {F0/kb:8.1f} K);  "
    ...
    axL.set_ylabel(r"$(F(z)-F(0))/k_B$ (K)", fontsize=25, fontweight="bold")
    axL.set_xlim(LEFT_PLOT_MIN_Z * X_SCALE, LEFT_PLOT_MAX_Z * X_SCALE)
    style_axes(axL, x_major=0.5, x_minor_div=5, y_minor=True)  # 0.5 nm majors, 0.1 nm minors
    ...
    axR.set_ylabel(r"$(F(z)-F(0))/k_B$ (K)", fontsize=25, fontweight="bold")
